...
class_mark_tbl <- tibble::tibble(
  assessment = class_mark_order,
  percentage = c(25, 20, 10),
  date = c("1 September", "9 September", "TBC (+/- Week 10)")
)
...
assessment_plot <- function(
  ...
  data$assessment <- factor(
    data$assessment,
    levels = rev(assessment_order)
  )

  p <- ggplot(
    data,
    aes(x = percentage, y = assessment, fill = assessment)
  ) +
    geom_col(width = 0.8) +
    geom_text(
      aes(x = percentage / 2, label = paste0(percentage, "%")),
      size = 5
    ) +
...
p_class_mark <- assessment_plot(
  class_mark_tbl,
  class_mark_order,
  title = "Regression components of class mark",
  x_max = 70,
  date_x = 28
)
